...
#Immune_celltype_df <- data@Immune_celltype_df
#Immune_celltype_ann <- data@anno$Response #可以由下面的一行所代替。
#data@Immune_celltype_df <- Immune_celltype
Immune_celltype_df_ann = cbind(Immune_celltype,ann = data@anno$Sex)#分组数据提取，分组数据必须是两类。
if(length(unique(Immune_celltype_df_ann$ann)) == 2){
...
dat_unand <- pd_data-mean_sd_data[,1]#PD组减去SD组的均值，因为这里的PD和SD都是经过log2之后的数值，因此，log2（PD/SD_mean）变等同于
...
#绘制森林图
cols <- c( "down" = "green", "not" = "white","up" = "green")
sp <- c("down" = 25, "not" = 16,"up" = 24)
...
p + 
  ...
  geom_point(size=2,
             fill = "black", stroke = 1.5
  )+
  geom_vline(aes(xintercept = 0)) +
  xlab('Down from PD/SD <--- Log2 FC ---> Up from PD/SD') + 
  ylab(' ')+
